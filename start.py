...
def addbot():
    if request.method == 'POST':
        ...
        ip, port, login, password = request.form['ip'], request.form['port'], request.form['login'], request.form['password']
        ...
        try:
            ...
            tg.create_on_db(session.get('auth'), ip, port, login, password)
            time.sleep(2)
...
async def api_manager():
    ...
    if request.method == 'POST' and 'action' in request.form:
        ...
        if action == 'create_session':
            ...
            ip, port, login, password = request.form['ip'], request.form['port'], request.form['login'], request.form['password']
            Telegram.create_tmp_session(api_id, api_hash, phone, ip, port, login, password)
